...
export const GameOverview = () => {
        ...
        const [userVariableSelections, setUserVariableSelections] = useState<Record<string, string>>({})
        const [userLevelSlug, setUserLevelSlug] = useState('')
        ...
        // Derived: user's pick if valid, else first in list
        const selectedCategoryId = useMemo(
                () => catList.some(c => c.id === userCategoryId)
                        ? userCategoryId
                        : (catList[0]?.id || ''),
                [catList, userCategoryId],
        )
        ...
        // Derived: default variable selections from current category's variables
        const defaultVariableSelections = useMemo(() => {
        ...
        const variableSelections = Object.keys(userVariableSelections).length > 0
                ? userVariableSelections
                : defaultVariableSelections

        // Derived: user's level pick if valid, else first level
        const selectedLevelSlug = useMemo(() => {
                if (userLevelSlug && levels?.some(
                        l => (l.slug || l.id || slugify(l.name)) === userLevelSlug,
                )) {
                        return userLevelSlug
                }
                if (mode === 'per-level' && levels?.length) {
                        const first = levels[0]
                        return first.slug || first.id || slugify(first.name)
                }
                return ''
        }, [userLevelSlug, mode, levels])

        // Event handlers that reset dependent state
        const handleModeChange = (newMode: string) => {
                setMode(newMode as ModeType)
                setUserCategoryId('')
                setUserVariableSelections({})
                setUserLevelSlug('')
        }
        ...
        const handleVariableChange = (variableId: string, value: string) => {
                setUserVariableSelections(prev => {
                        const base = Object.keys(prev).length > 0 ? prev : defaultVariableSelections
                        return { ...base, [variableId]: value }
                })
        }
                                        ...
                                        {selectedCategory && (
                                                <div className="flex flex-col gap-3">
                                                                ...
                                                                return (
                                                                        <div key={variable.id} className="flex flex-col gap-2">
                                                                                ...
                                                                                {compact ? (
                                                                                        <div className="flex flex-wrap gap-1">
                                                                                                {values.map(v => {
                                                                                                        ...
                                                                                                        return (
                                                                                                                <button
                                                                                                                        key={v.value}
                                                                                                                        onClick={() => handleVariableChange(variable.id, v.value)}
                                                                                        ...
                                                                                        <div className="w-64">
                                                                                                <Select value={variableSelections[variable.id] || ''} onValueChange={(val) => handleVariableChange(variable.id, val)}>
                                                        ...
                                                        {mode === 'per-level' && levels && (
                                                                <Tabs value={selectedLevelSlug} onValueChange={setUserLevelSlug}>
